...
def read_population_data():
    # Read the CSV file using pandas, skipping the header rows
    print("Reading CSV file...")
    df = pd.read_csv('btw2025_strukturdaten.csv', delimiter=';', decimal=',', skiprows=9, encoding='utf-8-sig')
    ...
    for _, row in df.iterrows():
        try:
            district_number = int(row['Wahlkreis-Nr.'])
            # Multiply by 1000 as the values are in thousands
            population = int(float(str(row['Bevölkerung am 31.12.2023 - Insgesamt (in 1000)']).replace('.', '').replace(',', '.')) * 1000)
            citizens = int(float(str(row['Bevölkerung am 31.12.2023 - Deutsche (in 1000)']).replace('.', '').replace(',', '.')) * 1000)
            
            district_data[district_number] = {
                'population': population,
                'citizens': citizens
            }
            # Print the data for each district
            print(f"District {district_number}: {population:,} total population, {citizens:,} citizens")
        ...
        for gruppe in gebiet.findall('Gruppenergebnis'):
            if gruppe.get('Gruppenart') == 'PARTEI':
                ...
                for stimme in gruppe.findall('Stimmergebnis'):
                    ...
                    anzahl = stimme.get('Anzahl')
                    votes = int(anzahl) if anzahl != 'n/a' else 0
