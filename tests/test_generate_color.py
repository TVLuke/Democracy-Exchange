...
from load_parties import pick_color
...
def test_generate_color_from_name():
    party_names = [
        "AfD",  # Germany
        "B*",   # Germany
        "BP",    # Germany
    ...
    colors = [pick_color() for name in party_names]
    
    # Check that all colors are distinct
    for i in range(len(colors)):
        for j in range(i + 1, len(colors)):
            dist = color_distance(colors[i], colors[j])
            assert dist > 2, f"Colors for {party_names[i]} ({colors[i]}) and {party_names[j]} ({colors[j]}) are not distinct enough."
